...
|---|---|---|---|
| `[Nurul Aminatul]` | `[nim]` | `[kelas]` | `[ anggota ]` |
...
```text
[Alat: AI Assistant, panduan
Prompt ringkas: "Menyusun draf laporan M9 m9-scheduler dari template MCSOS berdasarkan berkas pendukung dan log history terminal"
Sumber: Model Bahasa Besar
...
| `` `Alya Syara Shafira` `` | `` `25832073009` `` | `` `Toolchain Engineer` `` | `` `Mengonfigurasi Makefile untuk rules build baru, mengelola branch Git, dan setup otomasi audit.` `` | `` `Makefile` `` |
| `` `Nisrina Amanda Puteri` `` | `` `25832072010` `` | `` `Verification Engineer` `` | `` `Merancang skenario unit test rotasi antrean thread hos, negative test, dan verifikasi stack layout.` `` | `` `tests/test_scheduler.c` `` |
...
|---|---:|---|---|
|  `` `Meyliza Rosmalia Putri` `` | `` `25%` `` | `` `kernel/sched/mcsos_thread.c` `` | `` `Menyelesaikan penulisan logika TCB dan manajemen antrean.` `` |
| `` `Alya Syara Shafira` `` | `` `25%` `` | `` `Makefile` `` | `` `Mengelola otomasi target build dan branching git ops.` `` |
| `` `Nisrina Amanda Puteri` `` | `` `25%` `` | `` `tests/test_scheduler.c` `` | `` `Menyelesaikan kasus uji validasi rotasi register CPU.` `` |
| `` `Nurul Aminatul Aliah` `` | `` `25%` `` | `` `scripts/check_m9_scheduler.sh` `` | `` `Menyusun skrip kepatuhan audit biner dan laporan.` ``  |
